...
def calculate_demographic_data(print_data=True):
    ...
    porcentagens_educação = df["education"].value_counts(normalize=True) * 100
    percentage_bachelors = round(porcentagens_educação['Bachelors'],1)
    ...
    rich_percentage = int((len(df.loc[(df['salary'] == '>50K') & (df['hours-per-week'] == 1)]) / num_min_workers) * 100)

    # What country has the highest percentage of people that earn >50K?
    highest_earning_country = ((df.loc[df['salary'] == '>50K'].groupby('native-country').size() / df.groupby('native-country').size() ) *100).idxmax()
